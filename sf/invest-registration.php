...
$result = $date->format('Y-m-d H:i:s');
	...
	$data = "\n".$leademail.",".$leadfname.",".$leadlname.",".$result.",".$leadpapad;//.",".$returnurl;
fwrite($handle, $data);
}
	...
	if ( $leadfname != "" && $leademail != "" ) {
		...
		$id = sfAddContact($leademail,  $leadfname, $leadlname, $leadtimez, $leadpapad);
		echo $id;

	} // end if we've got a name and email


function sfAddContact($leademail, $leadfname , $leadlname, $leadtimez, $leadpapad  )
{
	...
	if(count($Result->records)	>0){
		...
		$AccId = $Result->records[0]->Id;
			...
			$iLabQuery = "SELECT Id FROM Marketing_Communication__c WHERE Name = 'Investor Webinar - 13th May' AND Account__c = '".$AccId."'  LIMIT 1 ";
	...
	if(!$salesforceId){ //still not found them
		...
		// echo "here2";
			$iLabQuery = "SELECT Id FROM Marketing_Communication__c WHERE Name = 'Investor Webinar - 13th May' AND Account__c = '".$salesforceId."'  LIMIT 1 ";
			$Result2 = $Int->connection->query($iLabQuery);
				if(count($Result2->records)	>0){
					$guId = $Result2->records[0]->Id;
...
/* 	var_dump($affiliate); */
	...
	$sCusObject->$FieldName =  "Investor Webinar - 13th May";
	...
	// Add custom field
		...
		// echo "updating marketing Commun Application object";
			...
			//$sObject->RecordTypeId='01290000000DDQXAA4';//make a person account
			$sCusObject->Name				= "Investor Webinar - 13th May";
			$sCusObject->Account__c 		= $salesforceId;
