...
$leadcheck = "";
	...
	$leademail 		= $_REQUEST['email'];
	$leadphone 		= $_REQUEST['payment_mobile_country_code']. $_REQUEST['phonec'];
	$leadannual_turnover		= $_REQUEST['annual_turnover'];
	$leadsizet	= $_REQUEST['size_team'];
	// $leadbus_name = $_REQUEST['business_name'];
	// $leadcomp_Website = $_REQUEST['comp_Website'];
	// $leadpred_turnover = $_REQUEST['pred_turnover'];
	$leadwhy	= $_REQUEST['challenge'];
...
if ($handle) {
	...
	$data = "\n".$leademail.",".$leadfname.",".$leadlname.",".$leadphone.",".$leadannual_turnover.",".$leadsizet.",".$leadwhy.",".$result;
fwrite($handle, $data);
}
	...
	//	$retjson = httpPost($leademail, $leadfname , $leadlname, $leadtimez);
		...
		$id = sfAddContact($leademail, $leadfname, $leadlname,$leadphone,$leadannual_turnover,$leadwhy, $leadsizet );
...
function sfAddContact($leademail, $leadfname, $leadlname,$leadphone,$leadannual_turnover,$leadwhy, $leadsizet  )
{
	...
	if(count($Result->records)	>0){
		...
		// echo "here";
			$iLabQuery = "SELECT Id FROM GU_Service__c WHERE Name = 'GSols Comp - Business Activation Session Application' AND Account__c = '".$AccId."'  LIMIT 1 ";
	...
	if(!$salesforceId){ //still not found them
		...
		// echo "here2";
			$iLabQuery = "SELECT Id FROM GU_Service__c WHERE Name = 'GSols Comp - Business Activation Session Application' AND Account__c = '".$salesforceId."'  LIMIT 1 ";
			$Result2 = $Int->connection->query($iLabQuery);
				if(count($Result2->records)	>0){
					$guId = $Result2->records[0]->Id;
	...
	// $FieldName = 'Company'.$Postfix;
	// $sObject->$FieldName =  $leadbus_name;

	// $FieldName = 'top_3_links_for_websites_or_SM'.$Postfix;
	// $sObject->$FieldName =  $leadcomp_Website;
...
/* 	var_dump($affiliate); */
	...
	$FieldName = 'X2019_Revenue__c';
	$sCusObject->$FieldName =  $leadannual_turnover;


	// $FieldName = 'X2020_Revenue__c';
	// $sCusObject->$FieldName =  $leadpred_turnover;


	$FieldName = 'Genius_Solutions_Why_Join__c';
	$sCusObject->$FieldName =  $leadwhy;

	$FieldName = 'ESS_size_of_company__c';
	$sCusObject->$FieldName =  $leadsizet;

	$FieldName = 'Web_Form_application__c';
	$sCusObject->$FieldName =  "GSols Comp - Business Activation Session Application";
	...
	// Add custom field
		...
		echo "updating marketing Commun Application object";
			...
			//$sObject->RecordTypeId='01290000000DDQXAA4';//make a person account
			$sCusObject->Name				= "GSols Comp - Business Activation Session Application";
			$sCusObject->Account__c 		= $salesforceId;
